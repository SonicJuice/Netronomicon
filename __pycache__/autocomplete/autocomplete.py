...
(_partial if key.startswith(syn) else _clean).append(syn) 
...
def _update_words_with_partial_synonyms(self): 

""" 

helper method updates words with partial synonyms by adding new words to a dictionary 
...
temp_leaf_node = self._dawg.insert_dawg_node(word=word, normalised_word=normalised_word[:-1], add_word=add_word, original_key=original_key, count=count, insert_count=True) 
...
t				emp_leaf_node.children[last_char] = leaf_node # merge into leaf node if it doesn't have children 

else: 

leaf_node = self._dawg.insert_dawg_node(word=word, normalised_word=normalised_word, original_key=original_key, count=count, insert_count=True) 
...
results, find_steps = self._find_words(word, max_cost, size) 
...
output_items = [self._reverse_synonyms.get(item, item) if item not in self.words else item for item in output_items] 
...
rest_of_results, rest_find_steps = self._find_words(word=fuzzy_rest_of_word, max_cost=max_cost, size=size, call_count=call_count) 
...
self._add_descendant_words_to_results(node=fuzzy_new_node, size=size, matched_words=matched_words, results=results, distance=fuzzy_min_distance) 
...
self._add_descendant_words_to_results(node=new_node, size=size, matched_words=matched_words, results=results, distance=total_min_distance+1) 
