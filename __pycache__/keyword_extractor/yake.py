...
builds the contexts used to calculate relatedness. Words occurring within an n word window are considered as context words. Only words co-occurring in a block (sequence of words that appear in the vocabulary) are considered. 
...
computes the weight of individual words in terms of casing, position, frequency, relatedness to context, and occurrence in different sentences 
...
self.features[word]['POSITION'] = log( 

3.0 + median(sentence_ids)) 
...
# sets sum_ to -1+eps so 1+sum_ != 0 if the candidate is a one token stopword at the start or the end of the sentence 
...
""" tests if one candidate is redundant with respect to a list of already 

ones. A candidate is considered redundant if its 

Levenshtein distance with another candidate that is ranked higher in 

the list exceeds the pre-set threshold.  
